...
ext4FS myext4fs3;
ext4FS myext4fs4;
ext4FS *myext4fsp[] = {&myext4fs1, &myext4fs2, &myext4fs3, &myext4fs4};
...
// lets initialize a RAM drive.
  ...
  if(!myext4fs1.begin(sda1)) { // Setup first partition on sda.
    Serial.printf("myext4fs1.begin(%s) Failed: Drive plugged in?\n",mpName[sda1]);
  } else {
    Serial.printf("myext4fs1.begin(%s): passed...\n", mpName[sda1]);
    uint32_t estore = MTP.addFilesystem(myext4fs1, (const char *)myext4fs1.getVolumeLabel());
  ...
  if(myext4fs4.init_block_device(sd, 3) == EOK) {  
    Serial.printf("SD card is inserted...\n");
  } else {
    Serial.printf("SD card is NOT inserted...\n");
  }
  if(!myext4fs4.begin(sdd1)) { // Setup for second partition on sda (if there). 
    Serial.printf("myext4fs4.begin(%s) Failed: Drive plugged in?\n",mpName[sdd1]);
  } else {
    Serial.printf("myext4fs4.begin(%s): passed...\n", mpName[sdd1]);
    estore = MTP.addFilesystem(myext4fs4, (const char *)myext4fs4.getVolumeLabel());
